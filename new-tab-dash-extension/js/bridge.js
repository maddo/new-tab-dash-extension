...
function updateCacheTimer() {
  ...
  if (cacheTimer) {
    ...
    cacheTimer.textContent = `Cache expires in: ${minutesLeft}m`;
  }
}
...
async function init() {
  try {
    ...
    // Set initial connection statuses to loading
    setConnectionStatus('github', 'loading');
    setConnectionStatus('jira', 'loading');
